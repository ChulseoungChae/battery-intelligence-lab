#!/usr/bin/env python3
"""일별 압축 궤적 실험 CLI (daily / by_chg_mode).
...
로우(비압축) 실험은 scripts/run_raw.py 를 사용하세요.
"""
...
from src.config import CHG_MODES, DAILY_EXPERIMENTS  # noqa: E402
...
def _add_exp_arg(p: argparse.ArgumentParser) -> None:
    p.add_argument(
        "--exp",
        choices=DAILY_EXPERIMENTS,
        default="daily",
        help="daily | by_chg_mode",
    )
    p.add_argument(
        "--mode",
...
def _add_train_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument("--win-step", type=int, default=1)
...
def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(
        description="EV SOH PatchTST — 일별 압축 (daily / by_chg_mode)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    sub = ap.add_subparsers(dest="cmd", required=True)

    p_prep = sub.add_parser("prepare", help="build daily trajectories")
    _add_exp_arg(p_prep)
    p_prep.add_argument("--data-dir", type=Path, default=None)
    p_prep.add_argument("--chunksize", type=int, default=200_000)

    p_train = sub.add_parser("train", help="train PatchTST")
    ...
    p_plot.add_argument("--win-step", type=int, default=1)
    p_plot.add_argument("--epochs", type=int, default=60)
    p_plot.add_argument("--patch-len", type=int, default=4)
    p_plot.add_argument("--stride", type=int, default=2)
