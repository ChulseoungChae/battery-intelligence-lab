...
python scripts/run_raw.py plot    --L 100 --H 1
# 또는
python scripts/run_raw.py all --L 100 --H 1 --row-stride 100
```
...
| R5 | `raw` (row_stride=100) | 100 | 1 | 11819 | **100%** | **100%** | [`runs/L100_H1/`](./outputs/raw/runs/L100_H1/) |

공통 데이터: 차량 4대, 기간 2022-12-15 ~ 2023-08-31, CUDA.  
R1–R4는 일별 궤적(735행), R5는 로우 샘플 궤적(약 5.9만행, `win_step=5`).
...
```bash
# daily
python scripts/run_daily.py train --exp <exp> --L <L> --H <H>
# raw
python scripts/run_raw.py train --L <L> --H <H>
```
-->
...
- raw에서 H를 키우거나 시간 단위(분/시간)로 리샘플해 hold-last와 차별화
