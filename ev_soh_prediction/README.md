...
| 일별 궤적 | 735행 (`outputs/traj/daily_trajectories.csv`) |
| 학습 타깃 | `soh` (일별 median) |
...
```
data/*.csv  (초단위 BMS, 4대 차량)
    │
    ▼  scripts/build_trajectories.py
outputs/traj/daily_trajectories.csv  (일별 집계, cell_volt_mean 포함)
    │
    ▼  scripts/train_patchtst.py
    ├─ 슬라이딩 윈도우 (과거 L일 → H일 후 SOH)
    ├─ Leave-One-Vehicle-Out(LOVO) 검증
    └─ 전체 차량으로 최종 학습 → .pt / .json 저장
```
...
코드상 정의: `scripts/train_patchtst.py` 의 `FEATURES` 리스트.
...
ev_soh_aicar/
  data/                         # 원본 CSV (device별)
  scripts/
    build_trajectories.py       # 일별 궤적 생성
    train_patchtst.py           # PatchTST 학습·검증
    plot_predictions.py         # LOVO 예측 그래프
  src/
    patchtst.py                 # PatchTST 모델 정의
  outputs/
    traj/daily_trajectories.csv
    models/
      aicar_L14_H7_patchtst.pt
      aicar_L14_H7_patchtst.json
      lovo_metrics.csv
    figs/
      lovo_pred_timeseries.png
      lovo_pred_scatter.png
      lovo_pred_accuracy.png
  docs/figs/                    # README 미리보기용 그래프 복사
  requirements.txt
  README.md
```
...
### 1. `scripts/build_trajectories.py` — 초단위 → 일단위
...
**출력** `outputs/traj/daily_trajectories.csv` (하루 1행)
...
### 3. `scripts/train_patchtst.py` — 윈도우 만들고 학습·검증
...
| 파일 | 내용 |
|------|------|
| `aicar_L14_H7_patchtst.pt` | 학습된 가중치 |
| `aicar_L14_H7_patchtst.json` | 피처 목록 + 표준화용 mean/std (`mus`, `sds`, `mu_t`, `sd_t`) |
...
```bash
cd /data1/ev_soh_aicar
pip install -r requirements.txt

# 1) 일별 궤적 생성
python scripts/build_trajectories.py

# 2) PatchTST 학습 (기본 L=14, H=7, LOVO 포함)
python scripts/train_patchtst.py

# 옵션 예
python scripts/train_patchtst.py --L 21 --H 14 --epochs 80 --holdout-device 1241225226
python scripts/train_patchtst.py --skip-lovo          # LOVO 생략, 최종 학습만
```
...
상세: `outputs/models/lovo_metrics.csv`

### 예측 그래프 (LOVO)

재생: `python scripts/plot_predictions.py`  
원본 저장: `outputs/figs/` · README 미리보기용 복사본: `docs/figs/`

#### 1) 차량별 시계열 — 실제 vs PatchTST vs hold-last

![LOVO timeseries](docs/figs/lovo_pred_timeseries.png)

#### 2) 실제 vs 예측 산점도 (±1.0%p 밴드)

![LOVO scatter](docs/figs/lovo_pred_scatter.png)

#### 3) 오차 분포 · 허용오차 정확도

![LOVO accuracy](docs/figs/lovo_pred_accuracy.png)
...
| 저장 | 경로 |
|------|------|
| 가중치 | `outputs/models/aicar_L14_H7_patchtst.pt` |
| 메타 | `outputs/models/aicar_L14_H7_patchtst.json` |
...
| 경로 | 내용 |
|------|------|
| `outputs/traj/daily_trajectories.csv` | 일별 집계 궤적 |
| `outputs/models/aicar_L{L}_H{H}_patchtst.pt` | 모델 가중치 |
| `outputs/models/aicar_L{L}_H{H}_patchtst.json` | 피처 목록·표준화 통계·모델 설정 |
| `outputs/models/lovo_metrics.csv` | LOVO Acc@±tol / MAE / margin |
| `outputs/figs/lovo_pred_*.png` | LOVO 예측 그래프 |
| `outputs/figs/lovo_predictions.csv` | LOVO 시점별 예측값 |
